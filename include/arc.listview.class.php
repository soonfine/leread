...
class ListView
{
     ...
     * @return    string
     */
    function __construct($typeid, $uppage=1,$actype)
    {
        global $dsql,$ftp;
        $this->TypeID = $typeid;
        $this->actype = $actype;
        ...
        if(!$this->IsError)
        {
			...
			$this->Fields['content'] = ReplaceKeyword("",$this->Fields['content']);
			$this->Fields['description'] = ReplaceKeyword("",$this->Fields['description']);
			$this->Fields['zuozhe'] = ReplaceKeyword("",$this->Fields['zuozhe']);
    ...
    function CountRecord()
    {
        ...
        {
            $cquery = "SELECT COUNT(id) AS dd FROM `#@__archives` arc WHERE ".$this->addSql;
        ...
        //初始化列表模板，并统计页面总数
        if($this->actype=="download") $tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$GLOBALS['cfg_df_style']."/downlist.htm";//下载页模版
        else $tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$this->TypeLink->TypeInfos['templist'];
    ...
    function MakeHtml($startpage=1, $makepagesize=0, $isremote=0)
    {
        ...
        // }
		if(substr_count($this->Fields['typedir'],"/")>1)
		{
			$a="/html".str_replace(strrchr($this->Fields['typedir'],"/"),"",$this->Fields['typedir']);
			CreateDir($a);
		}
        $murl = '';
        if($makepagesize > 0)
    ...
    function MakePartTemplets()
    {
		...
		//SBYOU.NET 修改 栏目页静态样式
        $makeFile = $this->GetTruePath().'/'.$this->Fields['typedir'].'.html';
        ...
        for($i=0;$i<$row;$i++)
        {
            ...
            for($j=0;$j<$col;$j++)
            {
                if($row = $this->dsql->GetArray("al"))
                {
                    ...
                    $row['filename'] = $row['arcurl'] = $row['typedir']."/".$row['id'].".html";
                    $row['typeurl'] = $row['typedir']."/";
	...
	function ReplaceKeyword($kw,&$body)
    {
        ...
        $query = "SELECT * FROM #@__keywords WHERE rpurl<>'' ORDER BY aid DESC"; 
        $dsql->SetQuery($query);
        $dsql->Execute();
