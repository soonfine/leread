...
class TypeUnit
{
    ...
    function ListAllType($channel=0,$nowdir=0)
    {
        ...
        $this->dsql->SetQuery("SELECT id,typedir,typename,ispart,sortrank,ishidden,overdate FROM `#@__arctype` WHERE reid=0 order by sortrank,id desc");
        $this->dsql->Execute(0);
        while($row = $this->dsql->GetObject(0))
        {
			...
			if($id==45 || $id==375 || $id==376){
				$sbyou_net_Style=" style='display:none'";
			}else{
				$sbyou_net_Style="";
			}
			
            echo "<table".$sbyou_net_Style." width='100%' border='0' cellspacing='0' cellpadding='2'>\r\n";
            //普通列表
            if($ispart==0)
            {
                if($this->GetTotalArc($id)==0)
				{
					if($overdate!="0")
						$sumact = "<font color='red'>(文档：".$this->GetTotalArc($id)."-已完结)</font>";
					else
						$sumact = "<font color='red'>(文档：".$this->GetTotalArc($id)."-连载中)</font>";
				}
				else
				{
					if($overdate!="0")
						$sumact = "(文档：".$this->GetTotalArc($id)."-已完结)";
					else
						$sumact = "(文档：".$this->GetTotalArc($id)."-连载中)";
				}
				echo "  <tr>\r\n";
            ...
            else if($ispart==1)
            {
                ...
                echo "    </td><td align='right'>";
                echo "<a href='{$GLOBALS['BOOK_URL']}".ltrim(($row->typedir),'/')."/' target='_blank'>预览</a>";
                echo "|<a href='catalog_do.php?cid={$id}&dopost=listArchives'>内容</a>";
                echo "|<a href='catalog_add.php?id={$id}'>增加子类</a>";
                echo "|<a href='catalog_edit.php?id={$id}'>更改</a>";
                echo "|<a href='catalog_do.php?dopost=moveCatalog&typeid={$id}'>移动</a>";
    ...
    function LogicListAllSunType($id, $step)
    {
        global $cfg_admin_channel, $admin_catalogs;
        $fid = $id;
        $this->dsql->SetQuery("SELECT id,reid,typedir,typename,ispart,sortrank,ishidden,overdate FROM `#@__arctype` WHERE reid='".$id."' order by sortrank,id desc");
        ...
        if($this->dsql->GetTotalRow($fid)>0)
        {
            while($row = $this->dsql->GetObject($fid))
            {
                ...
                if($ispart==0)
                {
					...
					{
						if($overdate!="0")
							$sumact = "<font color='red'>(文档：".$this->GetTotalArc($id).")</font><font color='red'>【完本】</font>";
						else
							$sumact = "<font color='red'>(文档：".$this->GetTotalArc($id).")</font>【连载】";
					}
					else
					{
						if($overdate!="0")
							$sumact = "(文档：".$this->GetTotalArc($id).")<font color='red'>【完本】</font>";
						else
							$sumact = "(文档：".$this->GetTotalArc($id).")【连载】";
					}
                    ...
                    echo "|<a href='catalog_edit.php?id={$id}'>更改</a>";
                    echo "|<a href='catalog_do.php?dopost=moveCatalog&typeid={$id}'>移动</a>";
					//echo "|<a href='catalog_do.php?dopost=unitCatalog&typeid={$id}'>合并</a>";
					//echo "|<a href='/download/maketxt.php?bookid={$id}'>生成txt文件</a>";
                    echo "|<a href='catalog_del.php?id={$id}&typeoldname=".urlencode($typeName)."'>删除</a>";
                    echo "&nbsp; <input type='text' name='sortrank{$id}' value='{$rank}' style='width:25px;height:20px'></td></tr></table></td></tr>\r\n";
                }
