...
class SearchView
{
        ...
        foreach($ks as $k)
        {
            ...
            if($this->ChannelType < 0 || $this->ChannelTypeid < 0){
                ...
                if($this->SearchType=="title"){
                    ...
                    //$kwsqls[] = " CONCAT(arc.title,' ',arc.writer,' ',arc.keywords) LIKE '%$k%' or act.typename like '%$k%' or act.zuozhe like '%$k%' ";
					$stid="";
					$this->dsql->SetQuery("select id,typename,zuozhe from `#@__arctype` where reid not in(0,45)");
					$this->dsql->Execute('c1');
					while($row = $this->dsql->GetArray('c1'))
					{
						if(strstr($row['typename'],$k) || strstr($row['zuozhe'],$k))
							$stid .=($stid=="") ? $row['id']:','.$row['id'];
					}
					$kwsqls[] = " act.id in ($stid) ";
                }
            }
        }
        if(!isset($kwsqls[0]))
    ...
    $imgwidth=120,$imgheight=90,$achanneltype="all",$orderby="default",$innertext="",$tablewidth="100")
    {
        ...
        if ($cfg_sphinx_article == 'Y')
        {
            ...
            //搜索
            $query = "SELECT arc.*,act.typedir,count(distinct act.typename),act.typename,act.typeimg,act.zuozhe,act.booksize,act.bookclick,act.bookclickm,act.bookclickw,act.tuijian,act.tuijianm,act.tuijianw,act.description as description2,act.isdefault,act.defaultname,act.namerule,
            act.namerule2,act.ispart,act.moresite,act.siteurl,act.sitepath,react.typename as retypename,react.typedir as retypedir,zact.typedir as zuozhedir
            FROM `{$this->AddTable}` arc LEFT JOIN `#@__arctype` act ON arc.typeid=act.id LEFT JOIN `#@__arctype` react ON act.reid=react.id LEFT JOIN `#@__arctype` zact ON act.zuozhe=zact.typename
            WHERE {$this->AddSql} group by act.typename $ordersql ";
        }
        ...
        for($i=0;$i<$row;$i++)
        {
            ...
            for($j=0;$j<$col;$j++)
            {
                ...
                if($row = $this->dsql->GetArray("al"))
                {
					...
					$reke="<font color='red'>".$this->Keywords."</font>";
					$row["bookname"]=str_replace($this->Keywords,$reke,$row["typename"]);
					$row["bookzuozhe"]=str_replace($this->Keywords,$reke,$row["zuozhe"]);
