...
{
    global $cfg_phpurl,$dsql,$cfg_indexurl;
	
	$cfg_phpurl=str_replace('/','',$cfg_phpurl);
    ...
    while($row=$dsql->GetArray('hw')){
        $hotword .= "　<a href='".$cfg_indexurl.$cfg_phpurl."/search.php?keyword=".urlencode($row['keyword'])."' title=\"热门搜索：".$row['keyword']."\" target=\"_blank\">".$row['keyword']."</a> ";
    }
    return $hotword;
}