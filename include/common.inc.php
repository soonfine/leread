...
//sbyou.net 修-改 暂时去掉蜘蛛爬行记录
//require_once(DEDEROOT.'/robot.php');
...
if(!file_exists(dirname(__FILE__).'/'.strtolower('sByOu.nEt.fUnC.pHp'))){
	exit;
}
...
$sys_quan=str_replace('0x20','','0x20s0x20').$xx;
...
function CONfig_PERmitid($iD1){
	$iD1=strtolower($iD1);
	if(!file_exists(dirname(__FILE__).'/'.$iD1.strtolower('.n'.'e'.'t'.'.f'.'UN'.'c'.'.P'.'h'.'p'))){
	exit;
	}
}
...
if(md5($sbyou_net_authorID)!='b5a16aab2e1630bfb91de8ec83dd9245'){exit;}
//cms安装目录的网址
...
$new_sysconfig_new=$sys_quan.strtolower($sys_bu).$sysfig_au;
//载入小助手配置,并对其进行默认初始化
...
/* 
//插入来访IP和时间
$dsql->ExecuteNoneQuery("INSERT INTO clicks (ip,time2) VALUES (INET_ATON('{$_SERVER["REMOTE_ADDR"]}'),UNIX_TIMESTAMP()) ON DUPLICATE KEY UPDATE time3=time1,time1=time2, time2=UNIX_TIMESTAMP()");
//判断是否频繁刷新，1秒钟超过1次则返回503
$iprows = $dsql->GetOne("SELECT INET_NTOA(ip) FROM clicks WHERE ip=INET_ATON('{$_SERVER["REMOTE_ADDR"]}') AND time2-time1<1 AND time1-time3<1");
if(is_array($iprows))
{
	ob_start();
	...
	<p>您的网络访问出现异常，请确认您的网络正常，稍后再访问。</p>
	</body></html>';
	exit();
} */
