...
$nn=0;
$replacearray=array(" ","!","?","？","（","）","(",")","！","，",".",":","。","：","【","】");
if($channelid==99) $con1=(intval($co_novelcount)>=0) ? intval($co_novelcount):1;//每次采集小说封面数量
elseif($channelid==98) $con1=(intval($co_novelcount)>=0) ? intval($co_novelcount)*2:2;//每次采集小说封面数量(换站)
else $con1=(intval($co_artcount)>=0) ? intval($co_artcount):10;//每次采集小说章节数量
	...
	if($con1>0 || $eorid!=0)
	{
		...
		if(is_array($row))
		{
			...
			if($channelid<10)
			{
				...
				$duibisql="Select count(a.id) as dd From `#@__archives` a left join `#@__addonarticle` b on(b.aid=a.id) where REVERSE(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(a.title,' ',''),'!',''),'！',''),'，',''),'?',''),'(',''),')',''),'？',''),'（',''),'）',''),'。',''),'：',''),':',''),'.',''),'[',''),']',''),'【',''),'】','')) like '$c_title%' and a.typeid='$c_typeid' and b.body<>''";
				$crow = $dsql->GetOne($duibisql);
				if($crow['dd']>0)
				{
					$dsql->ExecuteNoneQuery("update `#@__co_htmls` set isexport=1,isdown=1 where aid='$aids' ");
					echo " 【<font color='red'>重复1-1-$a_title</font>】<br>";
					continue;
				}
			}
...
//更新当前采集规则还剩多少条没有采集，以便在规则自动合并时判断，剩余多余5条的就不要合并进去，免得影响采集效果
