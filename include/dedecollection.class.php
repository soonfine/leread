...
require_once(DEDEINC."/Snoopy.class.php");
@set_time_limit(0);
...
class DedeCollection
{
    ...
    function DownUrl($aid, $dourl, $litpic='', $issave=TRUE)
    {
		...
		$body = pr_str_replace($body); //add by pagerank

        //保存资料到数据库
        if($issave)
        {
            $query = " UPDATE `#@__co_htmls` SET dtime='".time()."',result='".filter_utf8_char(addslashes($body))."',isdown='1' WHERE aid='$aid' ";
        ...
        return filter_utf8_char(addslashes($body));
    }
...
{
	$ostr=mb_convert_encoding($ostr, "UTF-8","GBK");
	return mb_convert_encoding($ostr, "GBK", "UTF-8");
} */
