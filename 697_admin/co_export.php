		...
		if($channelid=="99")
		{
			...
			foreach ($dtp->CTags as $ctag)
			{
				...
				else if($itemName == 'overtag')
				{
					...
					{
						$templist="{style}/page.htm";
						$overdate=$pubdate;
						$isover="yes";
					}
					else
					{
						$templist="{style}/page2.htm";
					}
				}
			}
			if($title=="" || $writer=="")
					...
					if($row)
					{
						...
						if($oldtypeid!=0 && $oldtypeid!=$txt_typeid)
						{
							...
							if($row)
							{
								...
								if($old_templist=="{style}/page.htm")
									unlink($filename);
								else
