...
else if($dopost=="viewArchives")
{
    ...
    echo "<script language='javascript'>location.href='".$BOOK_URL.ltrim($arcurl,'/')."';</script>";
    exit();
}
/*--------------------------
//异步上传缩略图
...
else if($dopost=="delArchives")
{
    ...
    if($fmdo=='yes')
    {
		...
		$aids=str_replace("`",",",$qstr);
		$tids="";
		$dsql->SetQuery("SELECT typeid,id FROM `#@__arctiny` WHERE id in($aids) ");
		$dsql->Execute();
		while($row = $dsql->GetArray())
		{
			$tids.=($tids=="") ? $row['typeid']:",".$row['typeid'];
		}
		$qstrs = explode("`",$qstr);
        $okaids = Array();
        foreach($qstrs as $aid)
        ...
        $typeid2=0;
		$tids = explode(",",$tids);
        foreach($tids as $tida)
        {
            $typeid1=$tida;
			//当上一篇文章和当前文章不是同一个小说时，自动生成上一本小说的封面
			if($typeid2!=0 && $typeid1!=$typeid2 && $typeid1!=0)
			{
				//生成txt文件并更新封面和分类
				$murl=$cfg_basehost."/download/maketxt.php?bookid=".$typeid2;
				$file = fopen($murl,"r");
				fclose($file);
			}
			$typeid2=$tida;
		...
		{
			//生成txt文件并更新封面和分类
			$murl=$cfg_basehost."/download/maketxt.php?bookid=".$typeid1;
			$file = fopen($murl,"r");
			fclose($file);
		}
        ShowMsg("成功删除指定的文档，并成功更新封面！",$ENV_GOBACK_URL);
        exit();
    }
...
function hebing(){ }
------------------------------*/
else if($dopost=='hebing')
{
    require_once(DEDEADMIN."/inc/inc_catalog_options.php");
    AjaxHead();
    $query = "SELECT arc.* FROM `#@__archives` arc WHERE arc.id='$aid' ";
    $arcRow = $dsql->GetOne($query);
    $divname = 'hebing';
    echo "<div class='title' onmousemove=\"DropMoveHand('{$divname}', 225);\" onmousedown=\"DropStartHand();\" onmouseup=\"DropStopHand();\">\r\n";
    echo "    <div class='titLeft'>合并文章</div>\r\n";
    echo "    <div class='titRight'><img src='images/ico-close.gif' style='cursor:pointer;' onclick='HideObj(\"{$divname}\");ChangeFullDiv(\"hide\");' alt='关闭' title='关闭' /></div>\r\n";
    echo "</div>\r\n";
    echo "<form name='quickeditform' action='archives_do.php?dopost=hebingSave&aid={$aid}' method='post'>\r\n";
    echo "<table width='100%' style='margin-top:6px;z-index:9000;'>\r\n";
?>
<tr height='32'>
    <td width="60" class='bline'>&nbsp;章  节：</td>
    <td class='bline'>
        <input name="title" type="text" id="title" value="<?php echo $arcRow['title']; ?>" style="width:90%" />
    </td>
</tr>
<tr height='32'>
    <td width="60" class='bline'>&nbsp;合并到：</td>
    <td class='bline'>
        <select name="toaid" id="toaid" style="width:90%">
    <?php
    $dsql->SetQuery("SELECT * FROM `#@__archives` WHERE typeid=".$arcRow['typeid']." order by id asc");
    $dsql->Execute();
...
<tr height='32'>
    <td width="60" class='bline'>&nbsp;位  置：</td>
    <td class='bline'>
        <input type="radio" value="1" name="hbtype">之前&nbsp;&nbsp;
        <input type="radio" value="2" name="hbtype">之后
    </td>
</tr>
<tr height='32'>
    <td colspan='2' align='center' style='padding-top:12px'>
...
else if($dopost=='hebingSave')
{
	$backurl = !empty($_COOKIE['ENV_GOBACK_URL']) ? $_COOKIE['ENV_GOBACK_URL'] : '-1';
	$Row1 = $dsql->GetOne("SELECT a.title,b.body FROM `#@__archives` a left join `#@__addonarticle` b on(a.id=b.aid) WHERE a.id='$aid'");
	$Row2 = $dsql->GetOne("SELECT a.title,b.body FROM `#@__archives` a left join `#@__addonarticle` b on(a.id=b.aid) WHERE a.id='$toaid'");
	if($hbtype==1)
	{
		$hbtitle=$Row1['title']."&nbsp;".$Row2['title'];
		$hbbody=$Row1['body']."<br />".$Row2['title']."<br />".$Row2['body'];
	}
	elseif($hbtype==2)
	{
		$hbtitle=$Row2['title']."&nbsp;".$Row1['title'];
		$hbbody=$Row2['body']."<br />".$Row1['title']."<br />".$Row1['body'];
	}
	else
	{
		ShowMsg('请选择合并位置！', $backurl);
        exit();
	}
	if($dsql->ExecuteNoneQuery(" UPDATE `#@__addonarticle` a,`#@__archives` b SET b.title='$hbtitle',a.body='$hbbody' WHERE a.aid='$toaid' and b.id='$toaid' "))
	{
        ...
        ShowMsg('合并成功！', $backurl);
        exit();
	}
	else
	{
        ShowMsg('合并失败，请检查数据是否有误！', $backurl);
        exit();
	}
}
/*-----------------------------
