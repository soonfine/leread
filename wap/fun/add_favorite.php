...
if(!$memberid || $info_m['jointime']!=$regdate || $info_m['uname']!=$mname){
	echo '<div class="data msg"><a href="'.$MOBILE_URL.'login.php">抱歉，您还未登陆！[快速登陆]</a></div>';
	exit;
}
...
if($chid==1){
	...
	if(!$arcRow['id'])
	{
		...
		if($sRow['bid'])
		{
			echo '<div class="data msg"><a href="'.$MOBILE_URL.'adminm.php?action=favorites">您已经收藏过这本小说了，请查看收藏夹！[打开收藏夹]</a></div>';
		}else{
			$dsql->ExecuteNoneQuery("INSERT INTO `#@__member_stow`(mid,bid,addtime) VALUES ('$memberid','$aid','$time')");
			echo '<div class="data msg"><a href="'.$MOBILE_URL.'adminm.php?action=favorites">恭喜您，成功添加书签！[打开收藏夹]</a></div>';
		}
	}
}else{
